...
class Products extends CI_Controller {
	...
	public function index()
	{
		...
		$this->home_model->Set_where("pro_ser_slug='".$watch_url."' AND pro_ser_type='product' AND pro_ser_status='active'");
		$this->home_model->Set_groupby();
		$this->home_model->Set_orderby();
		$this->home_model->Set_limit();
		...
		$this->home_model->Set_free_where();
		$this->home_model->Set_sql("website_pro_ser.pro_ser_id,pro_ser_name,pro_ser_usd,pro_ser_inr,pro_ser_slug,img_alt,pro_ser_img,pro_ser_intro FROM website_pro_ser LEFT JOIN website_pro_ser_pages ON website_pro_ser_pages.pro_ser_id=website_pro_ser.pro_ser_id");
		$this->home_model->Set_where("website_pro_ser_pages.page_id='".$data['single_fetch_data'][0]['page_id']."' AND pro_ser_status='active' AND pro_ser_type='product' AND website_pro_ser_pages.pro_ser_id!='".$data['post_pages'][0]['pro_ser_id']."'");
		$this->home_model->Set_groupby();
		$this->home_model->Set_orderby();
		$this->home_model->Set_limit();
		$data['product_data']=$this->home_model->latest_post();
		$this->load->view('public/content_header',$data);
		$this->load->view("product/content_index_".$data['post_pages'][0]['design_type'],$data);
		$this->home_model->Set_free_where();
