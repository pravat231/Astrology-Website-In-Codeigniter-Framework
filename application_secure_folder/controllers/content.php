...
class Content extends CI_Controller {
	...
	public function index()
	{
		...
		$this->home_model->Set_free_where();
		$this->home_model->Set_sql("page_id,parent_id,title,content,meta_title,meta_desc,meta_keyword,design_type from website_pages");
		$this->home_model->Set_where("slug='".$watch_url."' AND page_status='publish'");
		$this->home_model->Set_groupby();
		$this->home_model->Set_orderby();
		$this->home_model->Set_limit();
		$data['post_pages']=$this->home_model->latest_post();
		$data['site_title']=$data['post_pages'][0]['title'];
		...
		$this->home_model->Set_where("website_pro_ser_pages.page_id='".$data['post_pages'][0]['page_id']."' AND pro_ser_type='service' AND pro_ser_status='active'");
		$this->home_model->Set_groupby();
		$this->home_model->Set_orderby();
		$this->home_model->Set_limit();
		$data['service_data']=$this->home_model->latest_post();
		$this->home_model->Set_free_where();
		$this->home_model->Set_sql("website_pro_ser.pro_ser_id,pro_ser_name,pro_ser_intro,pro_ser_usd,pro_ser_inr,pro_ser_slug,pro_ser_img,img_alt FROM website_pro_ser LEFT JOIN website_pro_ser_pages ON website_pro_ser_pages.pro_ser_id=website_pro_ser.pro_ser_id");
		$this->home_model->Set_where("website_pro_ser_pages.page_id='".$data['post_pages'][0]['page_id']."' AND pro_ser_type='product' AND pro_ser_status='active'");
		$this->home_model->Set_groupby();
		$this->home_model->Set_orderby();
		$this->home_model->Set_limit();
		$data['product_data']=$this->home_model->latest_post();
		global $pre_parent;
		$pre_parent=$data['post_pages'][0]['parent_id'];
		$data['list_pages']=$this->list_pages();
		$this->load->view('public/content_header',$data);
		$this->load->view("content/content_index_".$data['post_pages'][0]['design_type'],$data);
		$this->home_model->Set_free_where();
