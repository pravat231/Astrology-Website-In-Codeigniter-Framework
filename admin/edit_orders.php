...
if(is_numeric($order_id)){
	if(isset($_POST['submit_form'])){
		mysql_query("UPDATE website_order SET order_status='".$_POST['order_status_check']."' WHERE order_id='".$order_id."'");
	}
...
<table border="0" cellpadding="0" cellspacing="0" width="100%">
  ...
  <tr>
    <td width="48%"><table width="100%" border="0" cellspacing="0" cellpadding="0" style="border:2px solid #d6d6d6" bgcolor="#fafafa">
      ...
      <form name="order_status_form" action="" method="post">
      <tr>
        <td>&nbsp;</td>
        <td>Order Status</td>
        <td>&nbsp;</td>
        <td><select name="order_status_check">
          <option value="Complete"<?php if($row_sql->order_status=='Complete'){ ?> selected="selected"<?php } ?>>Complete</option>
          <option value="Pending"<?php if($row_sql->order_status=='Pending'){ ?> selected="selected"<?php } ?>>Pending</option>
          <option value="Processing"<?php if($row_sql->order_status=='Processing'){ ?> selected="selected"<?php } ?>>Processing</option>
          <option value="Closed"<?php if($row_sql->order_status=='Closed'){ ?> selected="selected"<?php } ?>>Closed</option>
        </select>
        <span  class="submit-button cm-button-main" style="margin-left:20px"><input type="submit" name="submit_form" value="Submit" /></span>
        </td>
      </tr>
      </form>
      <tr>
  ...
  <tr>
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0" style="border:2px solid #d6d6d6" bgcolor="#fafafa">
      ...
      <tr>
		...
		echo $billing_info['BILLER_COUNTRY'].'<br>';
		echo 'Paypal Email : '.$billing_info['BILLER_PAYPAL_EMAIL'];		
		?> 
        </strong></td>
        </tr>
      <tr>
